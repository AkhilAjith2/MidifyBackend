...
def merge_mei_batches(list_of_mei_files, output_file):
    """
    Merge multiple MEI files into one consolidated MEI file.

    Args:
        list_of_mei_files (list): List of paths to MEI files to merge.
        output_file (str): Path to the output MEI file.

    Returns:
        None
    """
    if not list_of_mei_files:
        raise ValueError("The list of MEI files is empty.")

    # Parse the first MEI file as the base document
    base_tree = etree.parse(list_of_mei_files[0])
    base_root = base_tree.getroot()

    # Find the <music> or main container element in the base MEI file
    music_section = base_root.find(".//{*}music")
    if music_section is None:
        raise ValueError(f"No <music> section found in {list_of_mei_files[0]}.")

    # Iterate through the remaining MEI files and append their contents
    for mei_file in list_of_mei_files[1:]:
        tree = etree.parse(mei_file)
        root = tree.getroot()

        # Find the <music> section in the current MEI file
        current_music_section = root.find(".//{*}music")
        if current_music_section is None:
            raise ValueError(f"No <music> section found in {mei_file}.")

        # Append all children of the <music> section to the base <music> section
        for child in current_music_section:
            music_section.append(child)

    # Write the merged MEI content to the output file
    with open(output_file, "wb") as f:
        f.write(etree.tostring(base_tree, pretty_print=True, xml_declaration=True, encoding="UTF-8"))

    print(f"Merged MEI file created successfully: {output_file}")